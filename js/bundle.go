...
func (bi *BundleInstance) getExported(name string) goja.Value {
	return bi.ModuleInstance.GetBindingValue(unistring.NewFromString(name))
}
...
func NewBundleFromArchive(
	...
	rt := goja.New()
	i := NewInitContext(logger, rt, c, compatMode, arc.Filesystems, arc.PwdURL)
...
func (b *Bundle) getExports(logger logrus.FieldLogger, mi goja.ModuleInstance, options bool) error {
	for _, k := range b.Module.GetExportedNames() {
		v := mi.GetBindingValue(unistring.String(k))
...
func (b *Bundle) Instantiate(logger logrus.FieldLogger, vuID uint64) (*BundleInstance, error) {
	...
	for _, name := range b.Module.GetExportedNames() {
		v := mi.GetBindingValue(unistring.String(name))
	...
	var instErr error
	jsOptions := mi.GetBindingValue(unistring.String(consts.Options))
...
func (b *Bundle) instantiate(
	...
	err = common.RunWithPanicCatching(logger, rt, func() error {
		return init.moduleVUImpl.eventLoop.Start(func() error {
			var errRun error
			mi, errRun = rt.CyclicModuleRecordEvaluate(b.Module, b.resolveModule)
			return errRun
		})
	})
...
func (b *Bundle) setInitGlobals(rt *goja.Runtime, init *InitContext) (unset func()) {
	...
	mustSet("require", func(specifier string) (interface{}, error) {
		...
		/* TODO this seems to no longer be needed but I will keep it for now :P
		if mi := rt.GetModuleInstance(m); mi != nil {
			switch i := mi.(type) {
			case *wrappedGoModuleInstance:
				return toESModuleExports(i.mi.Exports()), nil
			case *wrappedCommonJSInstance:
				return i.exports, nil
			default:
		...
		*/
		...
		_, err = rt.CyclicModuleRecordEvaluate(m, b.resolveModule)
		if err != nil {
			return nil, err
		}
