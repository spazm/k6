...
This folder contains experimental modules and their documentation. Some of the experimental functionalities' code lives in this folder itself, other like the experimental redis, or experimental websockets modules are imported and injected in k6 directly from their respective repository's go modules. 

## Redis

This is a Redis module for [k6](https://github.com/grafana/k6) containing a client library allowing users to interact with Redis directly from their k6 scripts. It was built with the objective to enable users to either load test their Redis infrastructure, or use Redis as a data store backing their load tests' script logic.

The library's API intends to be quite thorough and extensive, but is still limited to a set of functionalities the team judged useful in the context of k6. The intent isn't to provide a full-fledged Redis client, integrating the whole Redis API, but rather to empower our users to use Redis in use-cases we believe are relevant to k6. If any Redis functionalities were to be missing for your use-case, feel free to open an issue or pull request.

Users can import it in their scripts by using the following import statement:

```js
import redis from "k6/experimental/redis";
```

**This module is experimental**. It is not guaranteed to be stable, and may change without notice. We will keep it as simple as possible, but we may need to add more features in the future. Please kindly provide us your user feedback, and open an issue or pull request if you have any suggestions. 

### Usage

This module exposes a [promise](https://javascript.info/promise-basics)-based API. Unlike most other current k6 modules and extensions, who operate in a synchronous manner,
the Redis module operates in an asynchronous manner. In practice, this means that using the Redis client's methods won't block the execution of the test, and that the test will continue to run even if the Redis client is not ready to respond to the request.

One potential caveat introduced by this new execution model, is that to depend on operations against Redis, all following operations should be made in the context of a [promise chain](https://javascript.info/promise-chaining). As demonstrated in the examples below, whenever there is a dependency on a Redis operation result or return value, the operation, should be wrapped in a promise itself. That way, a user can perform asynchronous interactions with Redis in a seemingly synchronous manner.
...
You can find this example as a javascript file in the [k6 samples](https://github.com/grafana/k6/blob/master/samples/redis.js).
...
As mentioned previously, the redis module exposes a subset of Redis' [commands](https://redis.io/commands) the core team judged relevant in the context of k6 scripts.
